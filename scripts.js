...
function INSERTAR(val) {
    ...
    function drawcolor(renglon, columna, posicion) {
        if (turnos == 1) {
            document.getElementById("c" + posicion).style.background = '#f41215';
            tablero[renglon][columna] = 3
        } else {
            document.getElementById("c" + posicion).style.background = '#0000FF';
            tablero[renglon][columna] = -3
        }
    }

    function profundidad(val) {
        c1 = tablero[0][val - 1]
        c2 = tablero[1][val - 1]
        c3 = tablero[2][val - 1]
        c4 = tablero[3][val - 1]
        c5 = tablero[4][val - 1]
        c6 = tablero[5][val - 1]
        let i = 0; //Posicion dentro del renglon en el que se insertara la
        let col = val - 1
        while (i < 7) {
            switch (i) {
                case 0:
